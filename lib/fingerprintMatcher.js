...
async function getRecommendationsFromFingerprint(fingerprint, features, mood = '') {
  try {
    // Try to identify the song (but don't block recommendations if it fails)
    ...
    const formattedRecsPromises = lfmTracks.slice(0, 20).map(async (track, idx) => {
      ...
      // Try to get album art from MusicBrainz (more reliable than Last.fm)
      let albumCover = null;
      const artist = track.artist?.name || track.artist;
      const title = track.name;

      try {
        console.log(`Looking up MusicBrainz for: ${artist} - ${title}`);
        const mbResults = await searchSong(artist, title);

        if (mbResults && mbResults.length > 0) {
          // Get the first result with a release that has cover art
          for (const mbTrack of mbResults.slice(0, 3)) {
            if (mbTrack.releases && mbTrack.releases.length > 0) {
              const release = mbTrack.releases[0];
              const releaseId = release.id;

              // Try to get Cover Art Archive URL
              const coverArtUrl = `https://coverartarchive.org/release/${releaseId}/front`;

              // Test if the URL is accessible (Cover Art Archive returns 404 if no art exists)
              try {
                const testResponse = await fetch(coverArtUrl, { method: 'HEAD' });
                if (testResponse.ok) {
                  albumCover = coverArtUrl;
                  console.log(`Found MusicBrainz cover art for ${title}`);
                  break;
                }
              } catch (e) {
        ...
        console.log(`MusicBrainz lookup failed for ${title}:`, error.message);
      }

      // Fallback to Last.fm if MusicBrainz didn't work
      if (!albumCover && track.image && Array.isArray(track.image)) {
    ...
    // Wait for all MusicBrainz lookups to complete
    const formattedRecs = await Promise.all(formattedRecsPromises);

    // Filter out songs without album art
    const recsWithArt = formattedRecs.filter(track => track.albumCover);
    console.log(`Tracks with album art: ${recsWithArt.length}`);
    ...
    return {
      identification,
      recommendations: recsWithArt.slice(0, 10)
    };
