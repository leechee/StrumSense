...
import { searchSongsByFeatures } from './dynamicMusicSearch.js';
import fs from 'fs';
...
export async function generateSongRecommendations({ audioFeatures, mood, userId }) {
  ...
  // Search iTunes dynamically for songs matching the audio features
  let scoredSongs = await searchSongsByFeatures(enrichedFeatures);

  // Apply additional scoring based on user preferences and mood
  scoredSongs = scoredSongs.map(song => {
    let additionalScore = song.matchScore || 0;

    if (mood) {
      const moodBonus = calculateMoodMatch(mood, [song.estimatedMood]);
      additionalScore += moodBonus;
    }

    if (userProfile) {
      additionalScore += calculateUserPreferenceScore(song, userProfile);
    }

    return {
      ...song,
      matchScore: additionalScore,
    };
  });
  ...
  const topRecommendations = scoredSongs.slice(0, 10).map((song, index) => ({
    ...
    title: song.title,
    artist: song.artist,
    difficulty: song.difficulty || enrichedFeatures.estimatedDifficulty,
    chords: song.chords || [],
    // Only show tempo/key if we have actual data (not from iTunes estimation)
    keySignature: song.keySignature || null,
    tempo: song.tempo || null,
    capo: song.capo || 0,
    techniques: song.techniques || [],
    matchScore: Math.round(song.matchScore),
    matchReasons: generateDynamicMatchReasons(song, enrichedFeatures, mood),
...
function calculateUserPreferenceScore(song, userProfile) {
  ...
  if (prefs.genres && song.genre && prefs.genres[song.genre]) {
    score += 10;
  }

  if (prefs.moods) {
    // Handle both array (static songs) and string (dynamic songs)
    const songMoods = Array.isArray(song.mood) ? song.mood : (song.estimatedMood ? [song.estimatedMood] : []);
...
function generateDynamicMatchReasons(song, features, mood) {
  const reasons = [];

  // Genre matching
  if (song.genre && song.genre.toLowerCase().includes('acoustic')) {
    reasons.push(`Acoustic ${song.genre} genre`);
  }

  // Mood matching
  if (mood && song.estimatedMood === mood.toLowerCase()) {
    reasons.push(`Matches your ${mood} mood`);
  }

  // Vibe matching
  const songText = `${song.title} ${song.artist}`.toLowerCase();
  const vibeMatches = features.vibe.filter(v => songText.includes(v));
  if (vibeMatches.length > 0) {
    reasons.push(`Similar feel: ${vibeMatches.slice(0, 2).join(', ')}`);
  }

  // Note: We don't show tempo for iTunes songs since it's not available from the API

  // Mode matching
  if (features.mode === 'minor' && (songText.includes('sad') || songText.includes('melancholic'))) {
    reasons.push('Minor key vibe');
  }

  if (features.mode === 'major' && (songText.includes('happy') || songText.includes('bright'))) {
    reasons.push('Uplifting major key feel');
  }

  // Fingerstyle
  if (features.isFingerstyle && songText.includes('fingerstyle')) {
    reasons.push('Fingerstyle guitar');
  }

  // Default reason if no specific matches
  if (reasons.length === 0) {
    reasons.push('Popular acoustic recommendation');
  }
